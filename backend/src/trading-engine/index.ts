// Trading Engine - Orchestrates all trading logic
...
export class TradingEngine {
  ...
  processMT5Update(payload: MT5Payload): void {
    ...
    for (const ticket of this.openPositionTickets) {
      if (!currentTickets.has(ticket)) {
        ...
        if (dna) {
          ...
          this.dnaEngine.finalizeTradeDNA(
            ticket,
    ...
    // Update closed trades sync
    if (payload.closedTrades && Array.isArray(payload.closedTrades)) {
      payload.closedTrades.forEach(t => {
        if (!this.closedTrades.find(existing => existing.ticket === t.ticket)) {
          this.closedTrades.unshift(t);
          tradingLogger.info('New closed trade:', t.ticket);
          if (t.profit < 0) {
            const dir = (t.type === 'BUY' || t.type === 0) ? 'BUY' : 'SELL';
            this.cooldowns[dir] = Math.max(Date.now(), this.cooldowns[dir]) + 300000;
          }
        }
      });
      if (this.closedTrades.length > 100) {
        this.closedTrades = this.closedTrades.slice(0, 100);
      }
    }
    ...
    for (const pos of this.accountState.positions) {
      ...
      if (tsUpdate) {
        tradingLogger.info('TS update for', ticket);
